				...
				next: ({data}) => {
					if (this.getId() === stopId) {
						...
						this.arrivals = data.list.map(arrivalAndDeparture => {
							...
							const absoluteTime = arrivalAndDeparture.arrivalTime > currentMillis ? arrivalAndDeparture.arrivalTime : arrivalAndDeparture.departureTime;
							return new Arrival(
								arrivalAndDeparture.routeShortName ?? arrivalAndDeparture.routeLongName ?? "",
								arrivalAndDeparture.stopHeadsign ?? arrivalAndDeparture.tripHeadsign ?? "",
								deviationMinutes,
								...
								formatTime(absoluteTime - currentMillis),
							);
						});
						this.loading = false;
						this.addMinutesAfterLoading = false;
